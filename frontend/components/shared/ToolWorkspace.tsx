...
export function ToolWorkspace({ slug, accept, maxSizeMB, actionType, allowMultiple, title }: ToolWorkspaceProps) {
  ...
  const handleProcess = async (config?: Record<string, unknown>, customFile?: File) => {
    ...
    if (slug === "organize-pdf") {
      try {
        ...
        const job = await createJob({
          tool: "organize-pdf",
          inputFileIds: [v1File.id],
          options: config ? (config as Record<string, any>) : { pages: [{ sourcePage: 1, rotation: 0 }] },
        });
        ...
        while (active) {
          ...
          if (updatedJob.status === "COMPLETED") {
            ...
            if (updatedJob.outputFile?.originalName) {
              ...
              setResultFileName("organized-document.pdf");
            }
            ...
            active = false;
            throw new Error(updatedJob.errorMessage || "We couldn't organize this PDF. Please try again.");
          }
        }
      } catch (err: any) {
        console.error("Job processing error for organize-pdf:", err);
        setProcessError({
          message: err?.message || "We couldn't organize this PDF. Please try again.",
          isAuth: false,
        });
        setFlowState("error");
        return;
    ...
    if (slug === "watermark" || slug === "watermark-pdf") {
      try {
        if (!files || files.length === 0) {
          throw new Error("Please select a PDF file to watermark.");
        }
        ...
        const job = await createJob({
          tool: "watermark-pdf",
          inputFileIds: [v1File.id],
          options: config ? (config as Record<string, any>) : { type: "text", text: "CONFIDENTIAL" },
        });
        ...
        while (active) {
          ...
          if (updatedJob.status === "COMPLETED") {
            ...
            if (updatedJob.outputFile?.originalName) {
              ...
              setResultFileName("watermarked-document.pdf");
            }
            ...
            active = false;
            throw new Error(updatedJob.errorMessage || "We couldn't watermark this PDF. Please try again.");
          }
        }
      } catch (err: any) {
        console.error("Job processing error for watermark-pdf:", err);
        setProcessError({
          message: err?.message || "We couldn't watermark this PDF. Please try again.",
          isAuth: false,
        });
        setFlowState("error");
        return;
      ...
      try {
        ...
        while (active) {
          ...
          if (updatedJob.status === "COMPLETED") {
            ...
            active = false;
            throw new Error(updatedJob.errorMessage || "We couldn't protect this PDF. Please try again.");
          }
        }
      } catch (err: any) {
        console.error("Job processing error for protect-pdf:", err);
        setProcessError({
          message: err?.message || "We couldn't protect this PDF. Please try again.",
          isAuth: false,
        });
        setFlowState("error");
        return;
    ...
    if (slug === "unlock-pdf") {
      try {
        if (!files || files.length === 0) {
          throw new Error("Please select a PDF file to unlock.");
        }
        ...
        while (active) {
          ...
          if (updatedJob.status === "COMPLETED") {
            ...
            active = false;
            throw new Error(updatedJob.errorMessage || "We couldn't unlock this PDF. Please try again.");
          }
        }
      } catch (err: any) {
        console.error("Job processing error for unlock-pdf:", err);
        setProcessError({
          message: err?.message || "We couldn't unlock this PDF. Please try again.",
          isAuth: false,
        });
        setFlowState("error");
        return;
        ...
        while (active) {
          ...
          if (updatedJob.status === "COMPLETED") {
            ...
            if (updatedJob.outputFile?.originalName) {
              ...
              setResultFileName("repaired-document.pdf");
            }
            ...
            active = false;
            throw new Error(updatedJob.errorMessage || "We couldn't repair this PDF. Please try again.");
          }
        }
      } catch (err: any) {
        console.error("Job processing error for repair-pdf:", err);
        setProcessError({
          message: err?.message || "We couldn't repair this PDF. Please try again.",
          isAuth: false,
        });
        setFlowState("error");
        return;
